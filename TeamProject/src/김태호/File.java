...
import Final2.ReserveController;
import Final2.MemberController;
import Final2.Member;
import Final2.Reserve;

public class File {
	// 1. 필드
	// 1. 회원정보를 저장하는 파일의 경로
	private static String memberpath = "C:/Users/505/git/ezen_kth/TeamProject_hotel_kth/src/Final2/memberlist.txt";

	private static String reservepath = "C:/Users/505/git/ezen_kth/TeamProject_hotel_kth/src/Final2/reservelist.txt";
	...
	public static boolean filesave(int type) {
		try {
			...
			if (type == 2) { // 
				fileOutputStream = new FileOutputStream(reservepath);
				for (Reserve reserve : ReserveController.reservelist) {
					String outstring = reserve.geth_id() + "," + reserve.getH_phone() + ","+ reserve.getDate();
					fileOutputStream.write(outstring.getBytes());
				}
